...
class Create extends Component
{
    ...
    public function save()
    {
        $this->validate([
            ...
            'category_id' => 'required_if:category_mode,select',
        ...
        if ($this->category_mode === 'new') {
            ...
            $finalCategoryId = $catResult['data']->id; // Assuming Store returns Model in 'data'
        }
        ...
        $result = $this->ratesService->Store($rateRequest);

        if ($result['status']) {
            return redirect()->route('dashboards.apps.deliveries.rates.index')->with('success', 'Rate created successfully!');
        } else {
            $this->addError('name', 'Failed to save rate: ' . $result['msg']);
        }
    }
    ...
    public function render(): ViewContract|Factory
    {
        ...
        return view('dashboards.apps.deliveries.rates.components.create-form', [
