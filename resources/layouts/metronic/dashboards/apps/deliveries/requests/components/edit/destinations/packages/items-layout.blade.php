            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden" wire:key="packages-{{ $index }}">
                ...
                <div class="px-4 py-3 flex items-center justify-between cursor-pointer bg-gradient-to-r from-blue-50 to-transparent" wire:click="toggle({{ $index }})">
                    <span class="text-sm font-semibold text-gray-700">
                         #{{ $index + 1 }} - <span class="text-blue-600">{{ $package['name'] ?: __('dashboard.request.create.form.new_item') }}</span>
                         @if($package['qty']) <span class="ml-2 px-2 py-0.5 bg-blue-100 text-blue-700 text-[10px] rounded-full">{{ $package['qty'] }} {{ __('dashboard.request.create.form.qty_suffix') }}</span> @endif
                    </span>
